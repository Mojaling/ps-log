...
Worker로 전송합니다. 문제에 직접 입력한 풀이 코드는 팀 공유용으로 중앙 D1에 저장하지만,
개인 메모, GitHub PAT와 전체 `data.json`은 중앙 서버로 보내지 않습니다.
...
활동은 발생 즉시 보내고 실패하거나 서버에서 거절된 전송은 브라우저 outbox에서 1분마다 재시도합니다. 랭킹 화면을
보고 있을 때는 1분마다, 다른 화면에서는 5분마다 조회하며 숨겨진 탭에서는 조회를 멈춥니다.
브라우저 새로고침이나 네트워크 끊김 때문에 같은 활동이 다시 가도 서버가 한 번만 계산합니다. 로그인할 때는
당일 로컬 활동을 다시 대조하므로, 전송 전에 탭을 닫았던 문제 성공·복습도 자동으로 복구합니다.

랭킹 표의 팀원을 누르면 그 팀원이 등록한 문제의 사이트·번호·문제명·난이도·링크와 해결 상태를
볼 수 있습니다. 목록은 클릭할 때만 최신 10개를 가져오고 브라우저 메모리에 1분간 보관합니다.
풀이가 있는 문제의 **풀이 보기**를 누르면 별도 페이지에서 C++·Python·Java 문법 강조와 코드
복사를 사용할 수 있습니다. 목록 응답에는 코드 본문을 넣지 않고, 풀이 페이지를 열 때 선택한
1건만 요청합니다. 이미 공유한 풀이의 해시를 브라우저가 기억해 코드가 바뀐 경우에만 다시
전송하며, 풀이 코드는 문제당 UTF-8 64KB로 제한됩니다. 개인 메모, 복습 메모와 GitHub 데이터는
중앙 서버로 보내지 않습니다.
...
앱이 `data.json`을 읽고 자동 커밋할 때 사용합니다. 토큰은 현재 탭의 `sessionStorage`에만 저장되고
설정창을 다시 열어도 입력값으로 노출하지 않습니다. 탭 세션이 끝나면 다시 입력해야 합니다.
...
받는 주소는 앱이나 `data.json`에 넣지 않고 Worker의 `MAIL_TO` 시크릿으로만 관리합니다.
연결 테스트 후 저장합니다. 다른 기기나 새 탭 세션에서는 토큰을 다시 입력해야 합니다.
...
| `Internal server error` | Worker 로그에서 상태 코드와 설정 누락 여부 확인 |

외부 API 응답 본문에는 이메일이나 저장소 정보가 섞일 수 있어 HTTP 응답과 로그 어디에도
남기지 않습니다. Worker 로그에는 제공자명·상태 코드·안전한 요청 ID만 기록합니다.
`npx wrangler tail`이나 Cloudflare 대시보드의 Workers Logs에서 확인하세요.
...
관리 키도 깃허브 토큰과 마찬가지로 **현재 탭의 sessionStorage에만** 저장되고 설정창에
다시 표시되지 않습니다. 공용 PC에서는 넣지 마세요.
...
브라우저용 토큰과 관리 키는 탭 세션에만 보관됩니다. 장기 저장 위험은 줄지만 같은 오리진에서
악성 스크립트가 실행되는 동안에는 읽힐 수 있으므로 앱은 다음을 지킵니다.

- 문제 링크와 개념 노트의 링크는 `http`·`https`만 `<a>`로 만듭니다.
  (`javascript:` 같은 주소는 링크를 걸지 않고 표시만 남깁니다.)
- 노트의 사진은 래스터 `data:image/...`와 `http(s)` 주소만 렌더링하고 SVG 업로드는 거부합니다.
...
- 이메일 주소는 URL·브라우저 저장소·`data.json`에 넣지 않고 `MAIL_TO` Worker 시크릿으로만 둡니다.
- 팀 로그인 일회용 코드는 쿼리스트링 대신 URL fragment로 전달한 뒤 즉시 지웁니다.
- 팀 풀이 페이지 주소도 fragment를 사용하며, 코드 본문은 로그인한 팀원이 해당 풀이를 열 때만
  중앙 서버에서 1건씩 받아옵니다. 문제 입력란의 풀이 코드는 팀원에게 공개되는 정보입니다.

GitHub PAT만 지키면 끝나는 것은 아닙니다. Worker의 `GITHUB_TOKEN`, `RESEND_API_KEY`,
`CRON_KEY`, Cloudflare 배포 토큰과 팀 서버의 `GITHUB_CLIENT_SECRET`, `SESSION_PEPPER`,
`ADMIN_KEY`도 유출 시 각각 데이터 열람·메일 오용·관리 API 호출·배포 변조·세션 위조 위험이
있습니다. 모두 최소 권한과 만료 기간을 적용하고 Cloudflare secret으로 등록합니다.

### 알고 쓰는 남은 위험

- **`/__cron`에 레이트 리밋이 없습니다.** 64자리 임의 키라 현실적 위험은 낮지만, 키를
  맞히면 메일 발송이 트리거됩니다. 걱정되면 Cloudflare Rate Limiting 규칙을 하나 걸어 두세요.
- **세션 저장은 실행 중 XSS까지 막지는 못합니다.** 공용 PC에서는 PAT와 관리 키를 입력하지 말고,
  입력했다면 설정의 “이 탭의 키 지우기”를 누른 뒤 탭을 닫으세요.
- **`workers.dev` 계정 서브도메인은 공개 주소입니다.** 실명·이메일 아이디·사번을 쓰지 말고,
  이미 포함돼 있다면 Cloudflare에서 익명 서브도메인 또는 커스텀 도메인으로 교체하세요.
...
- 문제 번호·제목·사이트·난이도·결과와 C++·Python·Java 풀이 코드 기록
...
- 팀원의 풀이 코드를 별도 페이지에서 언어별로 강조하고 복사
