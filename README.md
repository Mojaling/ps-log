...
푼 문제를 기록하고, 틀린 문제는 3·7·21일 뒤에 다시 보도록 일정을 잡아 주는 개인용 웹 앱입니다. 문제 기록, 문제 사이트 바로가기, 월별 잔디, 폴더 트리로 정리하는 C++/Java/Python/포트폴리오 노트, 주간 일정표, GitHub 동기화와 복습 메일을 제공합니다.
...
6. 웹용 R/W 토큰과 이메일 Worker용 Read-only 토큰을 **미리 채워진 발급 화면**으로 안내하고 권한 검증
...
- 붙여넣은 웹용 토큰이 실제로 `data.json`을 **읽고 쓸 수 있는지 즉시 검증**합니다.
  권한이 모자라면 그 자리에서 알려 주므로, 배포를 마친 뒤에야 동기화 실패를 발견하는 일이 없습니다.
  쓰기 확인은 어느 브랜치에도 연결되지 않는 blob 하나를 만들어 보는 방식이라 기록에 커밋이 남지 않습니다.
- 웹용 토큰을 클립보드에 올려 줍니다. 12단계에서 웹 설정창에 그대로 붙여넣으면 됩니다.
  이 토큰은 `.env`에 저장하지 않고 검증 후 메모리에서 지우므로, **비밀번호 관리자 등 안전한 곳에
  직접 보관해야 합니다.** GitHub에서는 다시 볼 수 없습니다.
